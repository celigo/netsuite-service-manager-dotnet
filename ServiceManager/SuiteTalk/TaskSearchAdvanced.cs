...
{
	public partial class TaskSearchAdvanced : ISearchAdvanced
	{
		...
		{ 
			if (search is TaskSearch)
				this.criteria = (TaskSearch)search; 
			else
				throw new System.ArgumentException("Parameter should be of type TaskSearch", "search");
		}
		...
		public SearchRow GetColumns(bool create)
		{
			...
			{
				this.columns = new TaskSearchRow();
			}
			return this.columns;
		}
		...
		{ 
			if (columns is TaskSearchRow)
				this.columns = (TaskSearchRow)columns; 
			else
				throw new System.ArgumentException("Parameter should be of type TaskSearchRow", "columns");
		}
