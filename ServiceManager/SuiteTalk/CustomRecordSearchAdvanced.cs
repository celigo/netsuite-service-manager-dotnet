...
{
	public partial class CustomRecordSearchAdvanced : ISearchAdvanced
	{
		...
		{ 
			if (search is CustomRecordSearch)
				this.criteria = (CustomRecordSearch)search; 
			else
				throw new System.ArgumentException("Parameter should be of type CustomRecordSearch", "search");
		}
		...
		public SearchRow GetColumns(bool create)
		{
			...
			{
				this.columns = new CustomRecordSearchRow();
			}
			return this.columns;
		}
		...
		{ 
			if (columns is CustomRecordSearchRow)
				this.columns = (CustomRecordSearchRow)columns; 
			else
				throw new System.ArgumentException("Parameter should be of type CustomRecordSearchRow", "columns");
		}
