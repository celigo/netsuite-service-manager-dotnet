...
{
	public partial class PriceLevelSearchAdvanced : ISearchAdvanced
	{
		...
		{ 
			if (search is PriceLevelSearch)
				this.criteria = (PriceLevelSearch)search; 
			else
				throw new System.ArgumentException("Parameter should be of type PriceLevelSearch", "search");
		}
		...
		public SearchRow GetColumns(bool create)
		{
			...
			{
				this.columns = new PriceLevelSearchRow();
			}
			return this.columns;
		}
		...
		{ 
			if (columns is PriceLevelSearchRow)
				this.columns = (PriceLevelSearchRow)columns; 
			else
				throw new System.ArgumentException("Parameter should be of type PriceLevelSearchRow", "columns");
		}
