...
{
	public partial class MessageSearchAdvanced : ISearchAdvanced
	{
		...
		{ 
			if (search is MessageSearch)
				this.criteria = (MessageSearch)search; 
			else
				throw new System.ArgumentException("Parameter should be of type MessageSearch", "search");
		}
		...
		public SearchRow GetColumns(bool create)
		{
			...
			{
				this.columns = new MessageSearchRow();
			}
			return this.columns;
		}
		...
		{ 
			if (columns is MessageSearchRow)
				this.columns = (MessageSearchRow)columns; 
			else
				throw new System.ArgumentException("Parameter should be of type MessageSearchRow", "columns");
		}
