...
{
	public partial class IssueSearchAdvanced : ISearchAdvanced
	{
		...
		{ 
			if (search is IssueSearch)
				this.criteria = (IssueSearch)search; 
			else
				throw new System.ArgumentException("Parameter should be of type IssueSearch", "search");
		}
		...
		public SearchRow GetColumns(bool create)
		{
			...
			{
				this.columns = new IssueSearchRow();
			}
			return this.columns;
		}
		...
		{ 
			if (columns is IssueSearchRow)
				this.columns = (IssueSearchRow)columns; 
			else
				throw new System.ArgumentException("Parameter should be of type IssueSearchRow", "columns");
		}
