...
{
	public partial class BudgetSearchAdvanced : ISearchAdvanced
	{
		...
		{ 
			if (search is BudgetSearch)
				this.criteria = (BudgetSearch)search; 
			else
				throw new System.ArgumentException("Parameter should be of type BudgetSearch", "search");
		}
		...
		public SearchRow GetColumns(bool create)
		{
			...
			{
				this.columns = new BudgetSearchRow();
			}
			return this.columns;
		}
		...
		{ 
			if (columns is BudgetSearchRow)
				this.columns = (BudgetSearchRow)columns; 
			else
				throw new System.ArgumentException("Parameter should be of type BudgetSearchRow", "columns");
		}
