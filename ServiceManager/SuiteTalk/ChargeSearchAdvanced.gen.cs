...
{
	public partial class ChargeSearchAdvanced : ISearchAdvanced
	{
		...
		{ 
			if (search is ChargeSearch)
				this.criteria = (ChargeSearch)search; 
			else
				throw new System.ArgumentException("Parameter should be of type ChargeSearch", "search");
		}
		...
		public SearchRow GetColumns(bool create)
		{
			...
			{
				this.columns = new ChargeSearchRow();
			}
			return this.columns;
		}
		...
		{ 
			if (columns is ChargeSearchRow)
				this.columns = (ChargeSearchRow)columns; 
			else
				throw new System.ArgumentException("Parameter should be of type ChargeSearchRow", "columns");
		}
