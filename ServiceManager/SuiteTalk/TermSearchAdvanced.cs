...
{
	public partial class TermSearchAdvanced : ISearchAdvanced
	{
		...
		{ 
			if (search is TermSearch)
				this.criteria = (TermSearch)search; 
			else
				throw new System.ArgumentException("Parameter should be of type TermSearch", "search");
		}
		...
		public SearchRow GetColumns(bool create)
		{
			...
			{
				this.columns = new TermSearchRow();
			}
			return this.columns;
		}
		...
		{ 
			if (columns is TermSearchRow)
				this.columns = (TermSearchRow)columns; 
			else
				throw new System.ArgumentException("Parameter should be of type TermSearchRow", "columns");
		}
