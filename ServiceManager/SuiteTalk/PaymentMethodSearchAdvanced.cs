...
{
	public partial class PaymentMethodSearchAdvanced : ISearchAdvanced
	{
		...
		{ 
			if (search is PaymentMethodSearch)
				this.criteria = (PaymentMethodSearch)search; 
			else
				throw new System.ArgumentException("Parameter should be of type PaymentMethodSearch", "search");
		}
		...
		public SearchRow GetColumns(bool create)
		{
			...
			{
				this.columns = new PaymentMethodSearchRow();
			}
			return this.columns;
		}
		...
		{ 
			if (columns is PaymentMethodSearchRow)
				this.columns = (PaymentMethodSearchRow)columns; 
			else
				throw new System.ArgumentException("Parameter should be of type PaymentMethodSearchRow", "columns");
		}
