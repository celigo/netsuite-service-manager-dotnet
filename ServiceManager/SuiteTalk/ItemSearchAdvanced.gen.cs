...
{
	public partial class ItemSearchAdvanced : ISearchAdvanced
	{
		...
		{ 
			if (search is ItemSearch)
				this.criteria = (ItemSearch)search; 
			else
				throw new System.ArgumentException("Parameter should be of type ItemSearch", "search");
		}
		...
		public SearchRow GetColumns(bool create)
		{
			...
			{
				this.columns = new ItemSearchRow();
			}
			return this.columns;
		}
		...
		{ 
			if (columns is ItemSearchRow)
				this.columns = (ItemSearchRow)columns; 
			else
				throw new System.ArgumentException("Parameter should be of type ItemSearchRow", "columns");
		}
