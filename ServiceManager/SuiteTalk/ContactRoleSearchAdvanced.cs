...
{
	public partial class ContactRoleSearchAdvanced : ISearchAdvanced
	{
		...
		{ 
			if (search is ContactRoleSearch)
				this.criteria = (ContactRoleSearch)search; 
			else
				throw new System.ArgumentException("Parameter should be of type ContactRoleSearch", "search");
		}
		...
		public SearchRow GetColumns(bool create)
		{
			...
			{
				this.columns = new ContactRoleSearchRow();
			}
			return this.columns;
		}
		...
		{ 
			if (columns is ContactRoleSearchRow)
				this.columns = (ContactRoleSearchRow)columns; 
			else
				throw new System.ArgumentException("Parameter should be of type ContactRoleSearchRow", "columns");
		}
