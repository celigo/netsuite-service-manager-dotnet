...
{
	public partial class TopicSearchAdvanced : ISearchAdvanced
	{
		...
		{ 
			if (search is TopicSearch)
				this.criteria = (TopicSearch)search; 
			else
				throw new System.ArgumentException("Parameter should be of type TopicSearch", "search");
		}
		...
		public SearchRow GetColumns(bool create)
		{
			...
			{
				this.columns = new TopicSearchRow();
			}
			return this.columns;
		}
		...
		{ 
			if (columns is TopicSearchRow)
				this.columns = (TopicSearchRow)columns; 
			else
				throw new System.ArgumentException("Parameter should be of type TopicSearchRow", "columns");
		}
