...
{
	public partial class FileSearchAdvanced : ISearchAdvanced
	{
		...
		{ 
			if (search is FileSearch)
				this.criteria = (FileSearch)search; 
			else
				throw new System.ArgumentException("Parameter should be of type FileSearch", "search");
		}
		...
		public SearchRow GetColumns(bool create)
		{
			...
			{
				this.columns = new FileSearchRow();
			}
			return this.columns;
		}
		...
		{ 
			if (columns is FileSearchRow)
				this.columns = (FileSearchRow)columns; 
			else
				throw new System.ArgumentException("Parameter should be of type FileSearchRow", "columns");
		}
