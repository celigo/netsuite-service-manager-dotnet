...
{
	public partial class SupportCaseSearchAdvanced : ISearchAdvanced
	{
		...
		{ 
			if (search is SupportCaseSearch)
				this.criteria = (SupportCaseSearch)search; 
			else
				throw new System.ArgumentException("Parameter should be of type SupportCaseSearch", "search");
		}
		...
		public SearchRow GetColumns(bool create)
		{
			...
			{
				this.columns = new SupportCaseSearchRow();
			}
			return this.columns;
		}
		...
		{ 
			if (columns is SupportCaseSearchRow)
				this.columns = (SupportCaseSearchRow)columns; 
			else
				throw new System.ArgumentException("Parameter should be of type SupportCaseSearchRow", "columns");
		}
